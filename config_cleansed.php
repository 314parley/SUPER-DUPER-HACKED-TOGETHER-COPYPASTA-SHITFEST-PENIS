<?php
#only use this if you use a socket connection
...
if (!$cache_loaded) {
	// Database
		...
		$cf['KU_DBDATABASE']      = '711chan'; // Database... database. On SQLite this will be the path to your database file. Secure this file.
	...
	// Imageboard info
		$cf['KU_NAME']      = '711chan'; // The name of your site
		$cf['KU_SLOGAN']    = '<em>"Not again ;-;"</em>'; // Site slogan, set to nothing to disable its display
		$cf['KU_HEADERURL'] = '/banners/r.php'; // Full URL to the header image (or rotation script) to be displayed, can be left blank for no image
		$cf['KU_IRC']       = 'irc.711chan.org #711chan'; // IRC info, which will be displayed in the menu.  Leave blank to remove it
	...
	// Paths and URLs
		// Main installation directory
			...
			$cf['KU_WEBPATH']   = 'http://www.711chan.org'; // The path to the index folder of kusaba, without trailing slash. Example: http://www.yoursite.com
			$cf['KU_DOMAIN']    = '.711chan.org'; // Used in cookies for the domain parameter.  Should be a period and then the top level domain, which will allow the cookies to be set for all subdomains.  For http://www.randomchan.org, the domain would be .randomchan.org; http://zachchan.freehost.com would be zach.freehost.com
	...
	// CSS styles
		$cf['KU_STYLES']        = '711chan'; // Styles which are available to be used for the boards, separated by colons, in lower case.  These will be displayed next to [Home] [Manage] if KU_STYLESWIKUHER is set to true
		$cf['KU_DEFAULTSTYLE']  = '711chan'; // If Default is selected in the style list in board options, it will use this style.  Should be lower case
		...
		$cf['KU_MENUSTYLES']        = '711chan:futaba'; // Menu styles
		$cf['KU_DEFAULTMENUSTYLE']  = '711chan'; // Default menu style
	...
	// Post handling
		...
		$cf['KU_MARKDOWN']       = false; // Whether or not to enable markdown. currently buggy, so disable.
	...
	// Extra features
		...
		// HTML5?
        $cf['KU_HTML5GRAPH']      = false; // Whether or not to enable the generation of graphs using html5, false for normal stats
	...
	// Post-configuration actions, don't modify these
		$cf['KU_VERSION']    = 'v. SUPER-DUPER-HACKED-TOGETHER-COPYPASTA-SHITFEST-PENIS edition';
