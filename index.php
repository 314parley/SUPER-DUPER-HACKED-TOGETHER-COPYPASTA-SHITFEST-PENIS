<?php
function isUsingWWW(){
	if(!strpos($_SERVER['HTTP_X_HOST'], "www.")){
		return true;
	}else{
		return false;
	}
...
$dwoo_data->assign('slogan', KU_SLOGAN);
...
switch ($page) {
	...
	case 'banlist':
		$bans = $tc_db->GetAll("SELECT ip, reason, boards, at, `by`, COALESCE(until, 'nuncAA') as until FROM {$prefix}banlist WHERE `by` != 'board.php' AND `by` != 'SERVER' ORDER BY `at` DESC");
	...
	//assign to dwoo variables
	$dwoo_data->assign('activestaff',$resultstaff);
	//output the page
	$dwoo->output(KU_TEMPLATEDIR . '/front_stats.tpl',$dwoo_data);
	break;
	default:
		$isWWW = 
