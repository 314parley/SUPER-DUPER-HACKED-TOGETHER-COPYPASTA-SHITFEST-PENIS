...
if (!$manage_class->ValidateSession(true)) {
	...
	if ($manage_class->CurrentUserIsAdministrator() || $manage_class->CurrentUserIsModerator()) {
		...
		if($manage_class->CurrentUserIsAdministrator()) {
			...
			$tpl_links .= '<li><a href="manage_page.php?action=purgeoldreqs">' . _gettext('Purge Old Requests') . '</a></li>';
		}
	}
	else {
		$uid = $tc_db->GetAll("SELECT `id` FROM `" . KU_DBPREFIX . "staff` WHERE `username` = " . $tc_db->qstr($_SESSION['manageusername']));
