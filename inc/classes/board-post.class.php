...
class Board {
	...
	function RegeneratePages() {
		...
		if ($this->board['enablecatalog'] == 1 && ($this->board['type'] == 0 || $this->board['type'] == 2)) {
                        ...
                        '&#91;<a href="' . KU_BOARDSFOLDER . $this->board['name'] . '/">'._gettext('Return').'</a>&#93; <div class="replymode">'._gettext('Catalog Mode').'</div>' . "\n" .
                        '<div class="catalog"><div class="threads">' . "\n";
                        $catalog_page = '';
                        $results = $tc_db->GetAll("SELECT `id` , `subject` , `file` , `file_type` , `message` FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $this->board['id'] . " AND `IS_DELETED` = 0 AND `parentid` = 0 ORDER BY `stickied` DESC, `bumped` DESC");
                        ...
                        if ($numresults > 0) {
                                ...
                                foreach ($results as $line) {
                                        ...
                                        if ($row <= $maxrows) {
                                                ...
                                                $catalog_page .= '<div class="thread">' . "\n" .
                                                '<center><a href="' . KU_BOARDSFOLDER . $this->board['name'] . '/res/' . $line['id'] . '.html">';
                                                ...
                                                if ($line['file'] != '' && $line['file'] != 'removed') {
                                                        if ($line['file_type'] == 'jpg' || $line['file_type'] == 'png' || $line['file_type'] == 'gif') {
                                                                ...
                                                                $catalog_page .= '<img src="' . $file_path . '/thumb/' . $line['file'] . 's.' . $line['file_type'] . '" alt="' . $line['id'] . '" class="image" />';
                                                        } else {
                                                ...
                                                $catalog_page .= '</a></center><br />' . "\n" . '<center style="margin-top:-15px;"><small><i>R: ' . $replies . '</i></small>' . "\n" . '<br>';
                                                if ($line['subject'] != '') {
                                                        $catalog_page .= '<strong>' . $line['subject'] . '</strong>';
                                                }
                                                $catalog_page .= '<span class="catalogtext">'. $line['message'] . "\n" . '</span></center></div>' . "\n";
                                        }
                                }
                        } else {
                                $catalog_page .= '<td>' . "\n" .
...
class Post extends Board {
	...
	function Report() {
		...
		return $tc_db->Execute("INSERT INTO `".KU_DBPREFIX."reports` ( `board` , `postid` , `when` , `ip`, `reason` ) VALUES ( " . $tc_db->qstr($this->board['name']) . " , " . $tc_db->qstr($this->post['id']) . " , ".time()." , '" . md5_encrypt($_SERVER['HTTP_X_FORWARDED_FOR'], KU_RANDOMSEED) . "', " . $tc_db->qstr($_POST['reportreason']) . " )");
	}
}
