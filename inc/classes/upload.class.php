				...
				if ($imagefile_name != '') {
					...
					if (is_array($exists_thread)) {
						exitWithErrorPage(_gettext('Duplicate file entry detected.'), sprintf(_gettext('Already posted %shere%s.'),'<a href="' . KU_BOARDSPATH . '/' . $board_class->board['name'] . '/res/' . $exists_thread[0] . '.html#' . $exists_thread[1] . '">','</a>'));
									...
									$ft = strtolower($this->file_type);
					
					/******* extended image matching (only for images - skip svgs) ******/
					
					if ($ft == '.gif' || $ft == '.jpg' || $ft == '.png') {
					
						require_once('inc/func/images.php');
							
						// skip everything if there isn't already an image posted....
						if(($res2 = getLatestImage($board_class->board['id'])) !== false){
							
							$settings = array(
								//'allowboards' 	=> '711,b,in,nig,smoke,quinn,gif,id,po,RAGE,x,sp,fit,ass,cake,jb,mens,tr,test,v,womens,g,speak,pol',
								'allowboards' => '711,b,h,in,nig,smoke,webm,quinn,gif,id,po,RAGE,x,sp,fit,ass,cake,jb,mens,tr,womens,g,speak,pol',
								'hashfail' 		=> 6,			
								'blankfail'		=> 0,			
								'hashcache'		=> 4,
								'resourcesize'	=> 8,
							...
							$res1 = imagecreatefromstring(file_get_contents($_FILES['imagefile']['tmp_name']));
							$board = $board_class->board['name'];
							$hashfile = KU_ROOTDIR.$board.'/hashes.ini';
								
							if($handle = fopen($hashfile,'a+')){
								ImageTest($res1, $res2, $this->file_name, $board, $hashfile, $settings, false);
								fclose($handle);
							}
						}
					}	
					/******* end extended image matching ******/
						...
						if ($filetype_forcethumb == 0) {
							...
							/* Otherwise, use this script alone */
								...
								if($this->file_type == '.webm'){
									require_once(KU_BOARDSDIR.'inc/classes/webm.class.php');
									
									generateWebmPreview($_FILES['imagefile']['tmp_name'], KU_BOARDSDIR . $board_class->board['name'] . '/thumb/' . $this->file_name . 'webm.jpg', 200);//last parameter is thumb width
}
