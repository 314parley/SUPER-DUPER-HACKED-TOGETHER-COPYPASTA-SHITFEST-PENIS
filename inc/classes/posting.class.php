...
class Posting {
  ...
  function CheckReplyTime() {
        global $tc_db, $board_class;
                ...
                $result = $tc_db->GetOne("SELECT MAX(timestamp) FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $board_class->board['id'] . " AND `parentid` !=0 AND `ipmd5` = '" . md5($_SERVER['HTTP_X_FORWARDED_FOR']) . "' AND `timestamp` > " . (time() - KU_REPLYDELAY));
	...
	function CheckNewThreadTime() {
		...
		/* Get the timestamp of the last time a new thread was made by this IP address */
		$result = $tc_db->GetOne("SELECT MAX(timestamp) FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $board_class->board['id'] . " AND `parentid` = 0 AND `ipmd5` = '" . md5($_SERVER['HTTP_X_FORWARDED_FOR']) . "' AND `timestamp` > " . (time() - KU_NEWTHREADDELAY));
		/* If they have posted before and it was recorded... */
		if (isset($result)) {
			/* If the time was shorter than the minimum time distance */
	...
	function CheckBannedHash() {
		...
		if (isset($_FILES['imagefile'])) {
			if ($_FILES['imagefile']['name'] != '') {
				...
				if (count($results) > 0) {
						$bans_class->BanUser($_SERVER['HTTP_X_FORWARDED_FOR'], 'SERVER', '1', $results[0]['bantime'], '', 'Posting a banned file.<br />' . $results[0]['description'], 0, 0, 1);
	...
	function CheckBlacklistedText() {
		...
		foreach ($badlinks as $badlink) {
			if (stripos($_POST['message'], $badlink) !== false) {
				...
				$bans_class->BanUser($_SERVER['HTTP_X_FORWARDED_FOR'], 'board.php', 1, 3600, '', _gettext('Posting a blacklisted link.') . ' (' . $badlink . ')', $_POST['message']);
