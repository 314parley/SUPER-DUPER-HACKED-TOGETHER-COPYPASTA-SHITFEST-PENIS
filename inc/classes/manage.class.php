...
class Manage {
	...
	function CheckLogin() {
		...
		$results = $tc_db->GetAll("SELECT HIGH_PRIORITY `ip` FROM `" . KU_DBPREFIX . "loginattempts` WHERE `ip` = '" . $_SERVER['HTTP_X_FORWARDED_FOR'] . "' LIMIT 6");
		if (count($results) > 5) {
			...
			if (count($results) > 0) {
				if (empty($results[0]['salt'])) {
					if (md5($_POST['password']) == $results[0]['password']) {
						...
						$tc_db->Execute("DELETE FROM `" . KU_DBPREFIX . "loginattempts` WHERE `ip` < '" . $_SERVER['HTTP_X_FORWARDED_FOR'] . "'");
						...
						$tc_db->Execute("INSERT HIGH_PRIORITY INTO `" . KU_DBPREFIX . "loginattempts` ( `username` , `ip` , `timestamp` ) VALUES ( " . $tc_db->qstr($_POST['username']) . " , '" . $_SERVER['HTTP_X_FORWARDED_FOR'] . "' , '" . time() . "' )");
  ...
  function CheckToken($posttoken) {
	...
	function Logout() {
		global $tc_db, $tpl_page;
		management_addlogentry(_gettext('Logged out'), 1);
    ...
    function RootOnly() {
    $rootusers = array('parley','apu');
    $comma_separated = implode (", ", $rootusers);
        global $tc_db, $tpl_page;
                if (!$this->CurrentUserIsAdministrator() || in_array($_SESSION['manageusername'], KU_ROOT)) {
                        exitWithErrorPage("Permission denied. Contact $comma_separated to do this action.");
                }
        }
	...
	*/
    ...
    function statisticshtml5() {
        global $dwoo_data, $tc_db, $tpl_page;
        $data .= $this->makejava("day", 'Posts per board in past 24hrs');
        $data .= $this->makejava("week", 'Posts per board in past week');
        $data .= $this->makejava("postnum", 'Total posts per board');
        $data .= $this->makejava("unique", 'Unique user posts per board');
        $dwoo_data->assign('javascript', $data);
        $tpl_page .= '<h2>' . _gettext('Statistics') . '</h2><br />';
        $tpl_page .= '
                <canvas id="day" width="1560" height="600">[No canvas support]</canvas>
		<canvas id="week" width="1560" height="600">[No canvas support]</canvas>
		<canvas id="postnum" width="1560" height="600">[No canvas support]</canvas>
		<canvas id="unique" width="1560" height="600">[No canvas support]</canvas>
                <br/>
    ...
    function makejava($type, $text) {
        ...
        $source = "
            var $type = new RGraph.Bar('$type', [[$string2]]);
            $type.Set('chart.key', ['$text']);
            $type.Set('chart.key.position.y', 35);
            $type.Set('chart.key.position', 'gutter');
            $type.Set('chart.key.background', 'rgb(255,255,255)');
            $type.Set('chart.labels', [$string1]);
            $type.Set('chart.colors', ['#000', '#000']);
            $type.Set('chart.shadow', false);
            $type.Set('chart.shadow.blur', 15);
            $type.Set('chart.shadow.offsetx', 0);
            $type.Set('chart.shadow.offsety', 0);
            $type.Set('chart.shadow.color', '#aaa');
            $type.Set('chart.strokestyle', 'rgba(0,0,0,0)');
            $type.Set('chart.gutter.left', 55);
            $type.Set('chart.gutter.right', 5);
            $type.Draw();";
        return $source;
    }
	...
	*	Ban Requests
	*	+------------------------------------------------------------------------------+
	*	Since I'm rewriting this, it may look like an ugly piece of shit. 
	*	The first two functions are for the links in the management panel.
	*	after them, are the sub-functions.
	*/
	
	//this function handles the first link...
	function banreqs() {
		global $tc_db, $tpl_page;
		$this->ModeratorsOnly();
		$requests = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "ban_requests`");
		if(isset($requests[0][0])){
		/* If we have ban requests...*/
			$tpl_page .= '<h2>' . _gettext('Ban Requests') . '</h2>';
			$tpl_page .= '<ul>';
			foreach($requests as $request){
			//here's the logic behind the links... if the status is 0, stylize it, otherwise, remove the style.
				$tpl_page.='<li style="margin-left:25px;"><a href="manage_page.php?action=request&id=' . $request[0] . '"' . ($request['status'] == 0 ? ' style="font-weight:bold; color:#a60000;"' : '') . '>' . _gettext('Request #' . $request[0]) . '</a></li>';
			}
		}else{
		/* We don't have any ban requests...*/
			$tpl_page .= '<h2>' . _gettext('Ban Requests') . '</h2>';
			$tpl_page .= '<h3>' . _gettext('No ban requests') . '</h3>';
		}
	}
	
	//this function purges all requesrs (should be admins only tbqh)
	function purgereqs() {
		global $tc_db, $tpl_page;
		$this->AdministratorsOnly();
		if($tc_db->Execute('DELETE FROM `' . KU_DBPREFIX . 'ban_requests`')){
			$tpl_page .= '<h2>' . _gettext('Ban Requests') . '</h2>';
			$tpl_page .= '<h3>All ban requests purged.</h3>';
		}else{
			$tpl_page = '<h3>Ban Requests NOT purged. See parley.</h3>';
		}
	}
	function purgeoldreqs() {
		global $tc_db, $tpl_page;
		$this->AdministratorsOnly();
		if($tc_db->Execute('DELETE FROM `' . KU_DBPREFIX . 'ban_requests` WHERE `status` != 0')){
			$tpl_page .= '<h2>' . _gettext('Ban Requests') . '</h2>';
			$tpl_page .= '<h3>Old ban requests purged.</h3>';
		}else{
			$tpl_page = '<h3>Old ban Requests NOT purged. See parley.</h3>';
		}
	}
	/* 
	function request() {
		...
		elseif(isset($_GET['changestat'])) {
			...
			$mod_id = $tc_db->GetAll("SELECT `id` FROM `" . KU_DBPREFIX . "staff` WHERE `username` = " . $tc_db->qstr($_SESSION['manageusername']));
			...
			if($_GET['changestat'] == 1) {
				...
				$post = $post[0];
				$board = $tc_db->GetAll("SELECT `name` FROM `" . KU_DBPREFIX . "boards` WHERE `id` = " . $post['boardid'] . " LIMIT 1");
				...
				<script type="text/javascript">
					setTimeout(function(){ this.location = "' . KU_WEBPATH . '/manage_page.php?action=bans&banboard=' . $board[0][0] . '&banpost=' . $post['id'] . '"; }, 2500);
				</script>';
			}
			else {
			...
			if(empty($ban_request[0][0])) {
				...
				$tpl_page .= '<ul style="font-size:13px; font-family:Tahoma, sans-serif;">
					<li><a href="' . KU_WEBPATH . '/' . $board[0]['name'] . '/res/' . ($post['parentid'] < 1 ? $post['id'] : $post['parentid']) . '.html#' . $post['id'] . '" style="text-decoration:none;" target="_blank">Post #</a>' . $ban_request['post'] . ' on <a href="' . KU_WEBPATH . '/' . $board[0]['name'] . '" target="_blank">/' . $board[0]['name'] . '/</a></li>
				...
				<blockquote>
					<table border="0" cellpadding="3" cellspacing="2">
						<tr>
							<td>
								<a href="" style="border:none; text-decoration:none;" target="_blank"><img src="' . KU_WEBPATH . '/' . $board[0]['name'] . '/thumb/' . $post['file'] . 's.' . $post['file_type'] . '" alt="" title="' . $post['file_original'] . '.' . $post['file_type'] . '" style="border:none; text-decoration:none;" /></a>
							</td>
							<td>
	...
	function request(){
		...
		if(isset($_GET['id'])&& is_numeric($_GET['id'])){
			...
			$tpl_page .= '<h2>Ban Request #' . $_GET['id'] . '</h2>';
			if($ban_request['status'] == 0) {
				//$tpl_page .= 'status is zero';
				$tpl_page.='<ul style="font-size:13px; font-family:Tahoma, sans-serif;"><li><a href="' . KU_WEBPATH . '/' . $board[0]['name'] . '/res/' . ($post['parentid'] < 1 ? $post['id'] : $post['parentid']) . '.html#' . $post['id'] . '" style="text-decoration:none;" target="_blank">Post #</a>' . $ban_request['post'] . ' on <a href="' . KU_WEBPATH . '/' . $board[0]['name'] . '" target="_blank">/' . $board[0]['name'] . '/</a></li></ul>';
			}
		}
	}
	...
	*/
	...
	function news() {
        ...
        $disptable = true;
        $formval = 'add';
        $title = _gettext('News Management');
        if (isset($_GET['act'])) {
			if ($_GET['act'] == 'edit') {
				if (isset($_POST['news'])) {
          $this->CheckToken($_POST['token']);
	...
	function rules() {
        ...
        $disptable = true;
        $formval = 'add';
        $title = _gettext('Rules Management');
        if (isset($_GET['act'])) {
			if ($_GET['act'] == 'edit') {
	...
	function blotter() {
        ...
        if (isset($values['important']) && $values['important'] == 1)
	...
	/* see all current users with data (name, trip email, IP and postcount) */
	...
	function userstats(){
		...
		<script type="text/javascript">
			var t = setTimeout(refresh,120000);
		...
		$data = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "posts` WHERE `IS_DELETED` = 0 ORDER BY `timestamp` DESC");
		
		if(count($data)>0){

			$table = '<table id="iptable" class="tablesorter"><thead><tr><th>Count</th><th>Name</th><th>Trip</th><th>Email</th><th>IP</th><th>Last Posted At</th><th>Postcount</th></tr></thead><tbody>'. "\n";
			...
			foreach($data as $user){
				...
				$decip = trim(md5_decrypt($user['ip'], KU_RANDOMSEED));
				...
				if(count($triparray) == 0){
					...
					if($user['name'] == ''){
						$tabledata[$i]['name'] = 'Anonymous';
					}
					else{
						$tabledata[$i]['name'] = $user['name'];
					}
					
					$tabledata[$i]['tripcode'] = $user['tripcode'];
					$tabledata[$i]['email'] = $user['email'];
					
					$board = $tc_db->GetOne("SELECT `name` FROM `".KU_DBPREFIX."boards` WHERE `id`=".$user['boardid']);
					$latestpost = '/'.$board.'/res/';
					
					if($user['parentid'] == 0){
						$latestpost.=$user['id'];
					}
					else{
						$latestpost.=$user['parentid'].'.html#i'.$user['id'];
					}
					
					$tabledata[$i]['latestpost'] = timeDiff($user['timestamp']).' in <a href="'.KU_BOARDSPATH.$latestpost.'">/'.$board.'/</a>'; //(date("m/d/y @ H:i:s", $user['timestamp']).' in '.$latestpost);
					$tabledata[$i]['ip'] = $decip;
				}

				else{
					if(!in_array($decip,$triparray)){
			...
			foreach($tabledata as $data){
				...
				if($data['ip'] != ''){
					...
					$table.='<tr><td>'.$c.'</td><td>'.$data['name'].'</td><td>'.$data['tripcode'].'</td><td>'.$data['email'].'</td><td><a href="manage_page.php?action=searchip&ip='.$data['ip'].'&board=all">'.$data['ip'].'</a></td><td>'.$data['latestpost'].'</td><td>'.$upcarr[$data['ip']].'</td></tr>';
				}
			}
			
			$tpl_page.=$table.'</table>';
	...
	*/
	...
	function embeds() {
        ...
        $disptable = true;
        $formval = 'add';
        $title = _gettext('Embed Management');
        if (isset($_GET['act'])) {
			if ($_GET['act'] == 'edit') {
	...
	function boardopts() {
      ...
      $this->CheckToken($_POST['token']);
			if (!$this->CurrentUserIsModeratorOfBoard($_GET['updateboard'], $_SESSION['manageusername'])) {
				exitWithErrorPage(_gettext('You are not managing this board.'));
			}
